...
4. 좌측: 이번 달 캘린더 (공사=주황, 미팅=파랑, 업무=초록). 날짜를 터치하면 그날 일정이 큰 글씨로 확대되어 보입니다.
5. 우측: 미확인 전화·기타메모 알림. 카드를 터치하면 이름을 선택해 확인 처리할 수 있습니다. 완료된 항목은 방금 확인한 순서대로 위에 쌓입니다.
6. 24시간 넘게 미확인 상태인 카드는 은은하게 깜빡입니다.
7. 상단 "업무" 버튼으로 업무 입력 화면으로 바로 이동할 수 있습니다.
...
4. 대시보드에는 현재 작성자와 관련된 업무·일정/공사·전화메모가 한 화면에 모아서 보이고, 다른 사람이 내가 작성한 항목의 삭제를 요청한 경우 여기서 승인/거절할 수 있습니다.
5. 업무관리에서 새 업무를 등록하면 요청자는 자동으로 현재 작성자가 되고, 담당자는 별도로 지정할 수 있습니다. 항목을 클릭하면 상세 이력을 보고 이어서 진행상황을 추가할 수 있습니다.
6. 일정·공사 등록 시 날짜는 "기간"과 "개별 날짜"를 함께 조합해서 추가할 수 있고, 숫자만 입력해도 자동으로 년-월-일 형식이 채워지며 팝업 달력에서 주말·공휴일(대한민국 법정 공휴일 기준)이 색으로 표시됩니다. 시간도 선택 입력할 수 있고, 담당은 특정 팀원 외에 "전체"·"미지정"으로도 지정할 수 있습니다.
7. 모든 목록은 각 행의 🗑 아이콘이나 상세화면에서 삭제할 수 있습니다. 본인이 작성한 항목은 바로 삭제되고, 다른 사람이 작성한 항목을 삭제하려 하면 작성자에게 삭제 요청이 전달되어 작성자가 대시보드에서 동의해야 실제로 삭제됩니다.
